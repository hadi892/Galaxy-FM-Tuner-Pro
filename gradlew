...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    if [ "$GRADLE_CYGWIN_VERBOSE" = "true" ] ; then
        echo "split windows path into:"
        echo "  cygwin dir: $dir"
        echo "  mixed path: $CLASSPATH"
        echo "  cygpath arg: $CYGPATTERN"
        echo "  cygpath cmd: $CYGPATH_SCRIPT"
        echo "cygwin home is set to: $CYGWIN_HOME"
        echo
        # let windows deal with it first
        win_path=`cygpath --path --windows "$CLASSPATH"`
        echo "windows path is: $win_path"
        mkdir -p "$DIRNAME"
        $CYGPATH_SCRIPT "$win_path"
        result=$?
    ...
    # Add the Java args held in GRADLE_OPTS in to the command line
...
# Stop when "xargs" has processed all the arguments
exec "$JAVACMD" "$@"
